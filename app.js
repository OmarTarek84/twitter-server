...
socket.on("connection", (nsSocket) => {
  ...
  nsSocket.on("notification Sent", async (data) => {
    console.log(data);
    const foundUsers = await User.find({
      username: { $in: data.notificationTo },
    });
    const currentUser = await User.findOne({ username: data.notificationFrom });
    if (!foundUsers || foundUsers.length <= 0) return;

    let notificationText = "";

    if (data.type === "retweet") {
      notificationText = "retweeted one of your posts";
    } else if (data.type === "like") {
      notificationText = "liked one of your posts";
    } else if (data.type === "follow") {
      notificationText = "followed you";
    } else if (data.type === "newMessage") {
      notificationText = "sent a new message";
    ...
    foundUsers.forEach((user) => {
      ...
      nsSocket.in(user._id.toString()).emit("notification received", {
        type: data.type,
        text: currentUser.firstName + " " + currentUser.lastName + " " + notificationText,
        postId: data.postId,
        chatId: data.chatId,
        firstName: currentUser.firstName,
        lastName: currentUser.lastName,
        followUsername: currentUser.username,
